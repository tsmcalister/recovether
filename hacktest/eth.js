...
class EthWrapper {
    ...
    storeKeyFile(privateKey,password){
        fs.writeFile("./keyfile.recovether", this.encrypt(privateKey,password), function(err) {
            if(err) {
    ...
    getRecovetherBalance(){
        return this.recovether
                    .methods.balanceOf(this.web3.eth.accounts.wallet[0].address)
                    .call();
    }
    ...
    withdrawFunds(amount){
        return this.recovether
                    ...
                    .send({to: this.recovether._address,
    ...
    secureFunds(amount){
        ...
        return this.web3.eth.sendTransaction({to: this.recovether._address, 
                                                from:this.web3.eth.accounts.wallet[0].address,
    ...
    */
    ...
    initializeAccount(username, password,amount){
        ...
        const passwordHash = this.web3.utils.soliditySha3(password+salt.toString())
    ...
    changePass(newPassword,salt){//-salt
        const passwordHash = this.web3.utils.soliditySha3(newPassword+salt.toString())
        return this.recovether
                    .methods
        ...
        if (eventName == 'TokenCreation')
        {
            this.recovether.events.TokenCreation((error,result)=> {
                if(!error)
                        ...
                        if(result.returnValues[0] == this.web3.eth.accounts.wallet[0].address)
                        {
...
const erc20contract = {abi: abi, address: '0xd41d1b985afC67f9A9aF61E893DaEe45A9C10Ff5'};
